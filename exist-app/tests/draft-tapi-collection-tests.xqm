...
module namespace t="http://ahikar.sub.uni-goettingen.de/ns/tapi/collection-draft/tests";
...
import module namespace d="http://ahikar.sub.uni-goettingen.de/ns/tapi/collection-draft" at "../modules/draft-tapi-collection.xqm";
...
as xs:string {
    d:get-collection-string($collection-type)
};
...
function t:get-json($collection-type as xs:string) {
    d:get-json($collection-type, $tc:server)
};
...
function t:make-sequence($collection-type as xs:string) {
    d:make-sequence($collection-type, $tc:server)
};
...
function t:get-allowed-manifest-uris-mock-up-input-included() {
    ...
    return
        d:get-allowed-manifest-uris($collection-metadata) = "3rx14"
};
...
function t:get-allowed-manifest-uris-mock-up-input-excluded() {
    ...
    return
        d:get-allowed-manifest-uris($collection-metadata) = "3vp38"
};
...
as xs:string {
    d:make-id($tc:server, $colletion-type, $manifest-uri)
};
...
function t:make-format-type($tgmd-format as xs:string) {
    d:make-format-type($tgmd-format)
};
...
as xs:string {
    d:make-annotationCollection-uri($tc:server, $collection-uri)
};
